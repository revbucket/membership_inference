...
Section('training', 'Hyperparameters').params(
    ...
    gpu=Param(int, 'Which GPU to use', default=0), # Final A100 on DGX
    save_path=Param(str, 'location of path to save models',
                    default='/home/mgj528/grad/membership_inference/save/cifar100_mi_nminus1/resnet18'),
...
Section('data', 'data related stuff').params(
    ...
    dataseed=Param(int, 'which seed for data to use', default=1234),
    query_set=Param(str, 'comma-separated set of indices to include/exclude',
                    default=','.join(map(str, [47153, 8018, 46832, 606, 13626, 11856, 41698, 27284,
                                               11994, 15178, 37334, 38683, 25175, 23049, 2074, 32096,
                                               308, 47265, 10166, 44206, 5590, 11602, 34903, 10936, 25839,
                                               37377, 36522, 6413, 33746, 38356, 9613, 41840, 8754, 16712,
                                               26467, 25253, 23998, 21297, 14218, 38219, 12185, 41863, 43257,
                                               49243, 40532, 34022, 33329, 24562, 3412, 33213]))),
    # include_bool=Param(int, 'Whether to include or exclude query_set from trainset', required=True),
    mongo_db=Param(str, 'database name for the mongodb', default='lira'),
    mongo_coll=Param(str, 'collection name for the mongodb', default='resnet18_cifar100_nm1')
)
...
@param('data.dataseed')
@param('data.query_set')
def main(index, epochs=None, save_path=None, base_name=None,
         mongo_db=None, mongo_coll=None, dataseed=None, query_set=None):
    ...
    query_set = set([int(_) for _ in query_set.split(',')])
    indices = list(range(50 * 1000))


    np.random.seed(dataseed)
    np.random.shuffle(indices)
    pro_indices = set(indices[:(25 * 1000)])
    anti_indices = set(indices[(25 * 1000):])


    include_bool = index % 2
    if include_bool:
        pro_indices = list(pro_indices.union(query_set))
        anti_indices = [_ for _ in anti_indices if _ not in query_set]
    else:
        pro_indices = [_ for _ in pro_indices if _  not in query_set]
        anti_indices = list(anti_indices.union(query_set))
